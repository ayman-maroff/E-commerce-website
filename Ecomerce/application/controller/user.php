...
class user extends Controller
{
    ...
    function register()
    {
        ...
        if (
            isset($_POST['firstname'])  && isset($_POST['lastname'])  && isset($_POST['midlname'])  &&
            isset($_POST['email']) &&
            isset($_POST['phonenumber'])
                ...
                $midlname = trim($_POST['midlname']);
                $midlname=strip_tags($midlname);
            $gender = $_POST['gender'];
            $email = $_POST['email'];
            ...
            if((!preg_match("/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/" , $email)) || (!preg_match("/^[0-9][0-9][0-9][0-9]+$/" , $phonenumber))){
                ...
                $_SESSION['ref'] = "home/index";
                 ...
                 if ($firstname=="" || $lastname=="" || $midlname=="") {
                     $_SESSION['alert_text'] = "An error occured!";
                     $_SESSION['alert_code'] = "error";
                     $_SESSION['ref'] = "home/index";
            
                 } else {
                     $_return_value = $model_name->add($firstname, $lastname, $midlname, $phonenumber, $gender, $email, $hashedpass, $ROLE);
                     if( $_return_value=="1")
                     {
                        $_SESSION['alert_text'] = "SignUp Completed!";
                        $_SESSION['alert_code'] = "success";
                        $_SESSION['ref'] = "user/login";
                        require_once 'application/templates/header.php';
                     ...
                     elseif( $_return_value=="2")
                     {
                        $_SESSION['alert_text'] = "An error occured!";
                        $_SESSION['alert_code'] = "error";
                        $_SESSION['ref'] = "user/preparetoRegister";
                        require_once 'application/templates/header.php';
                        require_once 'application/views/home/index.php';
    ...
    function userprofileSE($userId)
    {
        ...
        if (empty($array)) {
            ...
            require_once 'application/views/seller-views/navbar.php';
            require_once 'application/views/seller-views/sidebar.php';
            require_once 'application/views/users/profile.php';
        }
    }
    function userprofileSW($userId)
    {
       ...
       if($array==0){
           ...
           foreach ($array as $index) {
               ...
               if ($index->Role_id == 4) {
                ...
                $_SESSION['roleid'] = 4;
                $_SESSION['username'] = $index->firstname . " " . $index->lastname;
                $_SESSION['rolename'] = "StoreWorker";
                $_SESSION['userid'] = $index->uid;
                $ArrayOfUserPermissions = array();
                $model_name2 = $this->loadModel('RolePermessionModel');
                $permissions = $model_name2->getUserPermissions(4);
                foreach ($permissions as $UserPermission) {
                    array_push($ArrayOfUserPermissions, $UserPermission->perm_name);
                }
            ...
            if ($index->Role_id == 5) {
                ...
                $_SESSION['roleid'] = 5;
                $_SESSION['username'] = $index->firstname . " " . $index->lastname;
                $_SESSION['rolename'] = "DelivaryWorker";
                $_SESSION['userid'] = $index->uid;
                $ArrayOfUserPermissions = array();
                $model_name2 = $this->loadModel('RolePermessionModel');
                $permissions = $model_name2->getUserPermissions(5);
                foreach ($permissions as $UserPermission) {
                    array_push($ArrayOfUserPermissions, $UserPermission->perm_name);
                }
            ...
            if (
                ...
                if ((!preg_match("/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/", $email)) || (!preg_match("/^[0-9][0-9][0-9][0-9]+$/", $phonenumber))) {
                    ...
                    if ($firstname=="" || $lastname=="" || $midlname=="") {
                        ...
                        $reutrn_value = $model_name->userupdate($user_id, $firstname, $lastname, $midlname, $phonenumber, $email);
                        if ($reutrn_value == "1") {
